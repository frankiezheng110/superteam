...
- `framework/inspector.md` for trace emission, run analysis, and improvement tracking
- `framework/reviewer.md` for review-stage quality checking, blocker escalation, and specialist profiles
...
- every run must produce a trace file at `.superteam/inspector/traces/<task-slug>.jsonl` — this is not optional
- the orchestrator emits stage-level trace events; individual agents emit their own event types per `framework/inspector.md`
- every completed, failed, or cancelled run must produce a Inspector report
- completed runs acknowledge Inspector problem records in `finish.md`; failed or cancelled runs acknowledge them in the terminal handoff or retrospective note
- skills must not suppress, fabricate, or alter trace events

## Reviewer Integration

- the reviewer owns the review stage and runs quality checks across 8 dimensions per `framework/reviewer.md`
- when the reviewer finds a blocker, it immediately emits an `escalation` event to the orchestrator — it does not wait for review stage close
- the orchestrator decides what to do with reviewer blocker reports: route to the appropriate agent, update the plan, or accept the risk
- reviewer specialist profiles (critic, tdd, acceptance, socratic, security) are activated internally — they are not separate agents
...
- polish workers do not weaken `reviewer` or `verifier` authority
