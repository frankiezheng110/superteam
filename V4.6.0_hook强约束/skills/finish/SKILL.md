...
- `.superteam/inspector/reports/<task-slug>-report.html` (generated during finish)
...
- **before writing `finish.md`**, trigger Inspector analysis:
  ...
  2. analyze per `framework/inspector.md`
  3. generate the Inspector reports at `.superteam/inspector/reports/<task-slug>-report.html` and `.superteam/inspector/reports/<task-slug>-report.md`
  4. update `.superteam/inspector/health.json` and `.superteam/inspector/insights.md`
  5. generate problem records and improvement directives in `.superteam/inspector/improvement-backlog.md`
...
- include a **Inspector Summary** section in `finish.md`:
  - reference the Inspector report path
  - list key findings and their severity
- acknowledge every Inspector problem record: `acknowledged`, `addressed`, or `disputed`
  - if any `critical` tickets exist, they must appear prominently
- synchronize `.superteam/state/current-run.json` with `status=completed`, `last_completed_stage=finish`, latest handoff, `learning_status`, `improvement_action`, `inspector_report_status=acknowledged`, and `inspector_open_problems`
