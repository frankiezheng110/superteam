...
These behaviors are the basis of the Inspector's tracking and final report. For each behavior:
...
The Inspector uses this registry to emit `behavior_observed` trace events during and after the run. The final Inspector report is a structured aggregation of these events only — no analysis added.

---

## How to Use This Registry

For each agent that participated in a run, the Inspector:
...
**B-IN-0: Reviewer logged Gate 4 entry in activity-trace.md**
- What to check: `activity-trace.md` contains an reviewer entry log section that: (a) cites Gate 4, (b) lists execution.md path, (c) confirms polish.md was checked (or states not present), (d) lists MUST items it will check against
- Evidence format: find the entry log section in activity-trace.md
...
## INSPECTOR (Self-Check)

The Inspector applies these checks to its own trace before generating the final report.

**B-RV-1: Continuity checkpoints were emitted for clarify, design, and plan stages**
- What to check: `activity-trace.md` contains inspector checkpoint entries for each of the first three stages
...
**B-RV-2: Inspector report was generated before finish.md was written**
- What to check: `.superteam/inspector/reports/<slug>-report.md` timestamp is earlier than `finish.md` timestamp
...
- What to check: every claim in the Inspector report either cites a trace event or cites a specific artifact location
...
At the end of a run, the Inspector aggregates all behavior observations into a compliance table:
...
This table is the authoritative source for the Inspector report. It contains no interpretation — only the raw observation results and their citations.
