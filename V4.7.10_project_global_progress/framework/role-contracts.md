...
| --- | --- | --- | --- | --- |
| `orchestrator` | move work through the right stage at the right time | routing, approval, escalation, final decision, learning closure, Inspector trace emission, Inspector analysis trigger | become the default implementer, self-verify results, skip trace emission | all |
...
| `reviewer` | own all project deliverable quality checking: correctness, plan fidelity, code quality, security, artifact completeness, test coverage, UI quality — and immediately report any blocker to orchestrator | review-stage synthesis, blocker framing, profile activation, immediate escalation to orchestrator on any blocker | self-verify final success, silently skip hard challenge gates, decide to interrupt the workflow (orchestrator's decision), evaluate team behavior (Inspector's domain) | `review` |
...
| `inspector` | audit team behavior and maintain continuity visibility across the first three stages without gaining blocker authority | continuity checkpoints, trace analysis, agent collaboration diagram, quantitative run metrics, behavioral problem records, cross-run insights | `clarify`, `design`, `plan`, `finish` |
...
The reviewer is the team's primary quality gate. It owns all project deliverable quality checking. Below is the complete checklist the reviewer must work through.
...
The reviewer produces `review.md` with:
...
## Reviewer vs Inspector: Authority Boundary

These two roles have fundamentally different domains and different authority over the workflow.

### Reviewer — Deliverable Quality, Immediate Reporting Obligation

The `reviewer` watches the **project and its deliverables**. Its domain is correctness, completeness, and quality of what is being built.

- The reviewer actively evaluates output: code, design, artifacts, behavior
- When the reviewer finds a blocker, it **must report immediately to the orchestrator** via an escalation — it does not hold findings until the review stage closes
- **Whether to interrupt, pause, or continue is the orchestrator's decision** — the reviewer's obligation ends at reporting
...
- The reviewer owns the answer to: "Is this deliverable good enough to ship?"

### Inspector — Team Behavior, Zero Interrupt Authority

The `inspector` watches the **team and its process**. Its domain is how agents collaborated, how efficiently the workflow ran, whether the first three stages stayed legible, and whether the team behaved correctly.

- The inspector observes trace events throughout the run and may leave continuity checkpoints during `clarify`, `design`, and `plan`
- When the inspector detects a problem — even a critical one — it **records it silently** and does not interrupt
- All inspector findings are delivered in the post-run report, never mid-run
- The inspector owns the answer to: "Did the team work correctly and efficiently?"

**The inspector never touches deliverable quality. The reviewer never touches team behavior metrics.**

## Reviewer Specialist Profiles

The `reviewer` agent owns the review stage and activates specialist profiles internally based on risk. These are not separate agents:
...
- `clarify` defaults to `orchestrator` + `planner` + `inspector`, with `analyst` when needed
- `design` defaults to `orchestrator` + `architect` + `researcher` + `inspector`; for `ui-standard`/`ui-critical`, also requires `UI Intent Owner function` and `framework/frontend-aesthetics.md`; for `ui-critical`, inject `designer` by default
- `plan` defaults to `orchestrator` + `planner` + `inspector`, with `architect` for boundary checks
- `execute` defaults to `executor`; for UI work, executor must reference `ui-intent.md` contracts and anti-pattern registry
- `review` defaults to `reviewer` (activates specialist profiles as needed); for UI work, `designer` participates as mandatory UI quality gate
- `verify` defaults to `verifier`
- `finish` defaults to `orchestrator` + `verifier` + `inspector`, with `writer` when delivery prose needs cleanup
