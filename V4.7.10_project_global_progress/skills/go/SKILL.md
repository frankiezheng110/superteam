...
## V4.7.0 First Action — Enter OR Mode

Before doing anything else for this task, derive a kebab-case `task-slug` from the user's input
(short, ASCII-only, dash-separated; e.g. `phase-4-s3-module`) and run:

```
python "${CLAUDE_PLUGIN_ROOT}/commands/cli/mode_cli.py" enter <task-slug>
```

This writes `.superteam/state/mode.json` with `mode=active`. From now on **the main session itself
is the Orchestrator** (see `framework/main-session-orchestrator.md`). Do not spawn
`superteam:orchestrator` — that subagent is deprecated. You decide stages, you call specialists
directly through the `Agent` tool.

If the CLI returns `ok=false` with reason `already_active`, an OR session is already running for
another task. Tell the user and stop — do not overwrite. They must `/superteam:end` first or
explicitly confirm overwriting.

After mode.json is written, create `.superteam/runs/<task-slug>/` (if missing) and proceed
into the `clarify` stage.
