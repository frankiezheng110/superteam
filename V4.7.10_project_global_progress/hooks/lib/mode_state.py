...
def enter(slug: str, *, source: str = "/superteam:go") -> tuple[bool, str]:
    ...
    V4.7.7: also sets project_lifecycle="running" — the single field stop
    hook reads. mode/ended_at retained as legacy fields for backward compat.
...
def end(reason: str = "user_command") -> bool:
    ...
    V4.7.7: also sets project_lifecycle="ended".
    """
...
def write_mode(data: dict[str, Any]) -> bool:
    """V4.7.7 — public atomic write for migration / pause / resume.

    Refreshes last_verified_at on every write. Returns False when no
    .superteam root is available. Internal callers (enter/end) keep using
    _atomic_write_json directly so they can build the document from scratch.
    """
    p = mode_path()
    if not p:
        return False
...
def project_lifecycle() -> str:
    """V4.7.7 — string-valued lifecycle field for diagnostics / display.

    Returns one of: 'running', 'paused', 'ended'. Defaults to 'running' for
    legacy or broken mode.json — V4.7.9 fail-closed default. The user
    mandate is no exceptions: missing / corrupt / legacy → still alive,
    OR still cannot self-stop.

    V4.7.9 — this string value is for human/log/diagnostic display only.
    Hook-level decisions must consult `is_project_alive()` instead so the
    hook sees a single boolean, never a multi-valued string comparison.
    """
    try:
        return read_mode().get("project_lifecycle", "running")
    except Exception:  # noqa: BLE001 — fail-closed: any read failure → alive
        return "running"


_STOP_BLOCK_THRESHOLD = 4
"""V4.7.9 — consecutive stop-block count above which the runtime safety
valve activates. Up to (threshold - 1) blocks per stop cycle BLOCK; the
threshold-th and beyond ALLOW (case-3 hook-bug rescue). Strong-guidance
reason text directs the model to break out of case-2 dumping within the
first 1-3 blocks, well before the valve trips.
"""


def stop_block_count() -> int:
    """V4.7.9 — number of consecutive BLOCK decisions in the current stop
    cycle. Persisted in mode.json so it survives across hook invocations
    in the same lifecycle window.
    """
    try:
        return int(read_mode().get("stop_block_count", 0))
    except Exception:  # noqa: BLE001
        return 0


def bump_stop_block_count(*, reset_to_one: bool) -> int:
    """V4.7.9 — increment (or reset) the consecutive stop-block counter.

    `reset_to_one=True` when stop_hook_active=false (new stop cycle).
    `reset_to_one=False` when stop_hook_active=true (same cycle continues).
    Returns the new count. Best-effort write — failure leaves prior value.
    """
    md = read_mode()
    if reset_to_one:
        new = 1
        ...
        try:
            new = int(md.get("stop_block_count", 0)) + 1
        except Exception:  # noqa: BLE001
            new = 1
    md["stop_block_count"] = new
    try:
        write_mode(md)
    except Exception:  # noqa: BLE001
        pass
...
def reset_stop_block_count() -> None:
    """V4.7.9 — clear the counter (e.g. after the safety valve trips, or
    after a successful tool_use signaling progress). Best-effort.
    """
    md = read_mode()
    if md.get("stop_block_count", 0) == 0:
        return
    md["stop_block_count"] = 0
    try:
        write_mode(md)
    except Exception:  # noqa: BLE001
        pass
...
def is_project_alive() -> bool:
    """V4.7.9 — single-bit state machine for stop-hook decision.

    True  ⇔ project is alive ⇔ OR is forbidden from self-stopping.
    False ⇔ project not loaded OR ended ⇔ stop allowed.

    Two paths to False (only):
      - mode.json file does not exist (project never loaded — OR concept
        does not apply, hook is a no-op)
      - project_lifecycle == 'ended' (set by /superteam:end or finish-stage
        auto-completion)

    Everything else returns True, including:
      - mode.json exists but lifecycle == 'running' / 'paused' / any other
        non-'ended' value
      - mode.json exists but field missing
      - mode.json exists but corrupt / unreadable
      - any read exception

    Fail-closed by user mandate: "没有任何例外规则,哪怕无限循环一亿次,
    也不允许中途停止".
    """
    try:
        p = mode_path()
        if p is None or not p.exists():
            # Project never loaded into the state machine. Not an "exit",
            # just absence of the OR concept in this directory.
            return False
        return project_lifecycle() != "ended"
    except Exception:  # noqa: BLE001 — defense-in-depth fail-closed
        return True


def pause(by: str = "user") -> bool:
    """V4.7.7 — pause a running project.

    project_lifecycle: running → paused. Records paused_at + paused_by.
    Idempotent on already-paused. Returns False when no mode.json exists.
    """
    md = read_mode()
    if not md:
        return False
    md["project_lifecycle"] = "paused"
    md["paused_at"] = _utcnow_iso()
    md["paused_by"] = by
    return write_mode(md)


def resume() -> bool:
    """V4.7.7 — resume a paused project.

    project_lifecycle: paused → running. Clears paused_at / paused_by.
    Returns False when no mode.json exists.
    """
    md = read_mode()
    if not md:
        return False
...
def append_spawn_log(*, subagent_type: str, agent_id: str | None, task_slug: str | None) -> None:
    ...
    V4.7.2 — record stamps turn_id so the Stop hook can answer "did the
    main session spawn anything in this response turn?".
    """
    if not task_slug:
...
def violations_in_window(seconds: int = 60) -> list[dict[str, Any]]:
    """V4.7.4 — return gate violations recorded within the last `seconds`.

    Used by the escalation check to detect tight clusters of blocks. The
    window is sliding (relative to wall-clock now), not turn-based, because
    the same hook can fire repeatedly within a single turn.
    """
    ...
    for line in lines:
        ...
        ts_raw = rec.get("ts", "")
        try:
            rec_ts = datetime.fromisoformat(ts_raw)
        except ValueError:
            continue
...
# ---------- per-turn marker (V4.7.2 Stop-hook self-stop guard) ----------
#
# Stop hook needs to answer: "did the main session spawn any specialist
# in *this* response turn?" Without that, the hook cannot tell apart
# (a) legitimate stop after a spawn cycle vs (b) the OR producing prose
# and ending response without delegating.
#
# Implementation: UserPromptSubmit writes a fresh turn_id; PostToolUse
# Agent stamps the same turn_id onto the spawn-log record; Stop hook
# compares the latest spawn-log turn_id against the current turn_id.

import uuid as _uuid


def _turn_path() -> Path | None:
    d = _state.state_dir()
    return d / "turn.json" if d else None


def begin_turn() -> str:
    """Called by UserPromptSubmit. Generates a new turn_id and persists it."""
    p = _turn_path()
    if not p:
        return ""
    turn_id = _uuid.uuid4().hex[:16]
    rec = {"turn_id": turn_id, "started_at": _utcnow_iso()}
    try:
        _atomic_write_json(p, rec)
    except OSError:
        pass
...
def current_turn_id() -> str:
    ...
    if not isinstance(data, dict):
        return ""
    return str(data.get("turn_id", ""))


def last_spawn_turn_id(slug: str | None = None) -> str:
    """Return the turn_id stamped on the most recent spawn-log entry, or ''.

    Iterates backwards over the JSONL so partial corruption near the head
    of the log does not lose the recent record.
    """
    p = spawn_log_path(slug)
    if not p or not p.exists():
        return ""
    ...
    for line in reversed(lines):
        ...
        try:
            ...
            continue
        return str(rec.get("turn_id", ""))
    return ""


def spawned_in_current_turn(slug: str | None = None) -> bool:
    """True iff the most recent spawn-log entry's turn_id matches the active turn."""
    cur = current_turn_id()
    if not cur:
        return False
    return last_spawn_turn_id(slug) == cur
