...
This document defines the **binary enforcement checklist** that hooks execute at every stage transition.
...
> **V4.6.0**: The checklist below is executed automatically by `hooks/gates/gate_agent_spawn.py` plus the 14 artifact validators. When you try to spawn the next-stage agent, the hook physically blocks you if any mandatory check fails. Inspector `gate_check_report` and `gate_decision_observed` trace events are auto-emitted by `hooks/post_agent/post_agent_trace_writer.py` — you do not need to manually request them. Agent Entry Logs are validated by `hooks/post_agent/post_agent_entry_log.py`. Override Protocol still applies (see bottom of this file) and is enforced by `gate_agent_spawn.py::check_override_scope`.
...
| 10 | Inspector continuity checkpoint for `clarify` stage is present in `activity-trace.md` | read activity-trace.md, find clarify checkpoint |
...
| 14 | Inspector continuity checkpoint for `design` stage is present in `activity-trace.md` | read activity-trace.md, find design checkpoint |
...
| 15 | Inspector continuity checkpoint for `plan` stage is present in `activity-trace.md` | read activity-trace.md, find plan checkpoint |
...
**Executor**: orchestrator runs this gate after reviewer completes review.
...
### 5C — Delivery Scope Completeness (Reviewer's mandatory check)
...
### 5D — TDD Gate (Reviewer's mandatory check)
...
| 7 | Reviewer did NOT declare TDD "N/A" without citing the orchestrator waiver | read review.md TDD gate section |
...
If delivery scope check section is absent (check 4): reviewer must add it before the gate is re-evaluated.
...
| 3 | Inspector report exists at `.superteam/inspector/reports/<slug>-report.md` | file exists on disk |

### 7B — Finish Content

| # | Check | How to verify |
|---|-------|---------------|
| 4 | `finish.md` contains acknowledgment of Inspector report (`inspector_report_acknowledged: true` or equivalent) | read finish.md, find acknowledgment |
...
### 7C — Inspector Report (Hard Requirement)

**finish.md CANNOT be written until `.superteam/inspector/reports/<slug>-report.md` exists.**

This is an absolute gate. The Inspector report must be generated by the inspector agent BEFORE orchestrator writes finish.md. There is no waiver for this requirement.

If the inspector report does not exist: trigger inspector post-run analysis NOW before continuing.

### Gate 7 Failure Action

If Inspector report is absent (check 3): halt finish. Trigger inspector agent to produce the report. Resume after report is written.
...
- Missing Inspector report before finish
- Unresolved BLOCK findings from reviewer
...
| Gate 7 | finish | 6 checks | Inspector report absent |
