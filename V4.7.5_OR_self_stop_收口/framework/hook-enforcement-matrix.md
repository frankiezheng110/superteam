# SuperTeam V4.7.5 · Hook Enforcement Matrix

> **本文件性质**: SuperTeam 所有 "应当 / 禁止 / 必须 / 不得" 规则的**唯一事实源**。任何规则要在 enforce，必须在本矩阵中登记并绑定一个 hook checker 或明确标记为 `.md-only`（仅作为 AI 思维引导）。
>
...
>
> **V4.7.5 增补 — 状态机驱动 self-stop guard**: V4.7.2 用 `spawned_in_current_turn()` 试图阻止 OR 在 G3 后中途停下，但只判"本 turn 有 spawn 就放行"，被"spawn → 拿结果 → 写 prose+a/b/c → 停下"模式绕过（SMS phase-4-s3-module D-06 实物）。V4.7.5 删掉所有启发式（时间戳 / 关键词扫描 / verdict 卡死检测）改用纯状态机判定：`mode=active && current_stage∈{execute,review,verify} && !is_subagent_running` → 强制 BLOCK。状态机由 `gate_main_session_scope.py` 的 state-machine BLOCK 列表保护：`mode.json` / `active-subagent.json` / `turn.json` / `spawn-log.jsonl` / `subagent-stop-log.jsonl` / `gate-violations.jsonl` / `bypass-log.jsonl` 任何 agent 都不得 Edit/Write，只能由 hook/CLI 通过 Python API 维护。`current-run.json` 由 `gate_stage_advance.py` 校验合法推进路径。逻辑是状态机说还在跑就不允许停下；除非状态机出错，OR 永远跑不掉。
>
> **分层原则**:
...
| A1.5 | orchestrator.md L115/L125 (V4.7.5) | G3 后 OR 必须自决推进 · 不允许中途停下推回用户 | `stop.py::_or_self_stop_check` | `mode=active && current_stage∈{execute,review,verify} && !is_subagent_running` → block Stop 事件 (纯状态机判定 · 无启发式) | 新增 (V4.7.5 替换 V4.7.2 `spawned_in_current_turn` 启发式) |
| A1.6 | V4.7.5 | finish 阶段 OR 写完产物后允许停下等用户确认退出 | `stop_finish_guard.py::check` | finish 阶段 inspector report / finish.md / retrospective 任一不全 → block；全合格 → 放行让主会话停下 | 沿用 V4.6 (注：与 A1.5 互补 · finish 不在 EXEC_CLASS) |
...
| A4.11 | V4.7 main-session-orchestrator.md | 主会话即 OR · 不得直接 Edit/Write 实质工作文件 (代码 / review.md / verify.md / polish.md / final.md / plan.md / design.md 等) · 必须 spawn specialist | `gate_main_session_scope.py::check` | mode=active && !is_subagent_running && file ∈ blocked basenames/code suffixes → block (PreToolUse Edit/Write/MultiEdit) | 新增 (V4.7.0) |
| A4.12 | V4.7.5 | 状态机控制文件由 hook/CLI 通过 Python API 维护 · 任何 agent 都不得 Edit/Write | `gate_main_session_scope.py::check` (state-machine BLOCK) | mode=active && file ∈ {`.superteam/state/mode.json`, `active-subagent.json`, `turn.json`, `gate-violations.jsonl`, `bypass-log.jsonl`, `subagent-stop-log.jsonl`, `runs/<slug>/spawn-log.jsonl`} → block (无 bypass · 即使 specialist 也拦) | 新增 (V4.7.5 配套 A1.5 防状态机伪造) |
