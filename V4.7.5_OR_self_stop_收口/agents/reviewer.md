...
## Output Discipline (V4.7.4 evidence requirement)

Every claim in `review.md` must be backed by evidence the user can re-verify:

- Cite each finding with `path/to/file:LINE` (or a line range).
- For test/build/run claims, paste the actual command and the actual stdout
  excerpt (the last ~10 lines is fine — full block in a code fence).
- "Looks good" / "seems fine" / "should work" are forbidden — replace with
  the specific file:line you read and what you observed.
- For UI work, attach a `mcp__chrome-devtools-mcp__take_screenshot` call or
  a `mcp__plugin_playwright_playwright__browser_snapshot` excerpt.

`hooks/gates/gate_subjective_language.py` already blocks subjective verbs
in `review.md`; this section formalizes the positive form.
