...
def _verification_verdict_pass(slug: str) -> bool:
    """Read verification.md and check for a PASS verdict line."""
    rd = _state.run_slug_dir(slug)
    if not rd:
        return False
    vf = rd / "verification.md"
    if not vf.exists():
        return False
    try:
    ...
    return bool(re.search(r"(?im)^\s*verdict\s*[:：]\s*PASS\b", text))
...
def check(tool_input: dict[str, Any]) -> tuple[bool, str]:
    ...
    needed_artifact = _PREDECESSOR_ARTIFACT[new_stage]

    missing: list[str] = []
    if not _spawn_log_has(slug, needed_spawn):
        missing.append(f"spawn-log 缺少 superteam:{needed_spawn} 记录")
    if not _artifact_exists(slug, needed_artifact):
        missing.append(f"产物 {needed_artifact} 不存在")
    if new_stage == "finish" and not _verification_verdict_pass(slug):
        missing.append("verification.md 未携带 verdict=PASS")
    ...
    return False, (
        ...
        f"缺少：{'; '.join(missing)}\n"
        f"正确动作: 先 spawn superteam:{needed_spawn} 完成 {needed_artifact}，"
