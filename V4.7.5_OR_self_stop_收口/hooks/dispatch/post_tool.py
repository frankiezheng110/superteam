...
from hooks.lib import decisions, mode_state, parser, state  # noqa: E402
...
def _run_frontmatter_validator(file_path: str) -> None:
    """V4.7.3 — auto-stamp or audit specialist artifact provenance."""
    ok, errs = validator_frontmatter.run(file_path)
    if not ok:
        for e in errs[:3]:
            trace.emit_discrepancy("frontmatter", e, severity="medium")
...
def main() -> None:
    ...
    if tool == "Agent":
        # V4.7.0 — capture every main-session spawn for trust-chain audit
        if mode_state.is_or_active():
            ...
            mode_state.append_spawn_log(
                subagent_type=subagent_type,
                agent_id=str(agent_id) if agent_id else None,
                task_slug=mode_state.active_task_slug(),
            )
            # Close the subagent-running window (belt-and-suspenders with
            # SubagentStop — whichever fires first wins).
