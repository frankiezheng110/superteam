...
`inspector` must stay attached across the first three stages as a continuity auditor. This does not change Inspector into a stage owner or blocker source.
...
| `review` | run challenge-oriented quality gates before verification | `reviewer` | review artifact with blockers or clearance |
...
- `clarify -> design` only when `project-definition.md` clearly states what the project should do, direct user participation is recorded, explicit user approval is recorded, and Inspector continuity notes are current.
...
- `reviewer` remains the sole owner of the `review` stage
...
- `state/current-run.json` synchronization, Inspector trace events, and activity-trace continuity are all auto-maintained by hooks (`validator_current_run_json.py`, `post_agent_trace_writer.py`, `validator_activity_trace.py`).
...
- `completed` - verification passed, finish artifacts and Inspector report were produced
- `cancelled` - the workflow was intentionally stopped (partial Inspector analysis still required)
- `failed` - the repair loop was exhausted or a hard blocker could not be resolved safely (partial Inspector analysis still required)
