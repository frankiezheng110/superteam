...
| `finish` | PASS report, final artifacts, latest scorecard, Inspector report, `activity-trace.md` | final delivery summary, residual risks, follow-up options, archive-ready handoff, retrospective artifact, trace summary, Inspector summary, final UI intent outcome with aesthetic quality assessment when required | user or next operator | output is understandable without reopening prior context |
...
Reviewer treats every MUST item as a delivery contract. Absence = BLOCKER, not MINOR.
...
## Inspector Continuity Contract

The Inspector remains a passive auditor, but must leave continuity checkpoints across `clarify`, `design`, and `plan`.
...
- Inspector summary reference and acknowledgment status
...
`reviewer` owns the review stage by default and activates specialist profiles (critic, tdd, acceptance, socratic, security) internally based on risk. See `framework/role-contracts.md` → Reviewer Specialist Profiles for details.
