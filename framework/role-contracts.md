...
| `planner` | turn approved intent into executable task packages | scope framing, atomic tasks, commands, expected outputs, done signals, TDD steps, structural plan completeness | write production implementation or silently change architecture | `clarify`, `plan`, `execute` support, `fix` support |
| `architect` | define boundaries and technical shape | system boundaries, interfaces, decision rationale, risk framing | implement the solution or rubber-stamp vague design | `design`, `plan`, `fix` |
...
| `researcher` | collect source evidence and prior-art context | research-heavy tasks |
...
| `reviewer` | audit team behavior: data statistics, collaboration tracking, problem detection — post-run only, never interrupts | trace analysis, agent collaboration diagram, quantitative run metrics, behavioral problem records, cross-run insights |
...
The `reviewer` watches the **team and its process**. Its domain is how agents collaborated, how efficiently the workflow ran, and whether the team behaved correctly.

- The reviewer observes trace events passively throughout the run
...
- `clarify` defaults to `orchestrator` + `planner`, with `analyst` when needed
- `design` defaults to `architect`; for `ui-standard`/`ui-critical`, also requires `UI Intent Owner function` and `framework/frontend-aesthetics.md`; for `ui-critical`, inject `designer` by default
- `plan` defaults to `planner`, with `architect` for boundary checks
