...
For code-changing work, TDD is non-optional by default.

- the expected path is `red -> green -> refactor`
- no production implementation should be accepted before a failing test exists
- if implementation happened before the failing test, treat it as a TDD violation, not as a minor style issue
- the default correction is to return the run for rework unless the orchestrator records a narrow, explicit exception

## Post-Execute Polish Bridge

Before layer 2 (`inspector`), `SuperTeam` may run a refinement bridge:
...
- `inspector` must still independently judge whether the deliverable is good enough to continue
...
The Reviewer layer operates after delivery work is complete or terminated. For successful runs, this means after `verify` and before `finish.md`; for failed or cancelled runs, this means after the terminal decision. It does not judge the deliverable — it judges the team process:
...
The Reviewer produces its analysis as reports (`.superteam/reviewer/reports/<task-slug>-report.html` and `.superteam/reviewer/reports/<task-slug>-report.md`) and generates improvement records for any weaknesses found. See `framework/reviewer.md` for the full specification.
