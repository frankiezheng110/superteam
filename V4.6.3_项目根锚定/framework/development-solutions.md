...
- inspector continuity checkpoints
...
- `.superteam/inspector/traces/<task-slug>.jsonl` remains the machine-auditable source of truth
- `activity-trace.md` is a human-readable run summary for the first three stages, not a replacement for the Inspector trace
...
- `inspector` - continuity checkpoints, trace coverage, decision-trail integrity

Optional additions:

- `designer` when UI-bearing work materially affects solution direction
- `analyst` when the project definition is still unstable
- `reviewer` only when deliverable-quality concerns must be previewed early; this does not replace the real `review` stage
...
- inspector continuity checkpoints are captured
