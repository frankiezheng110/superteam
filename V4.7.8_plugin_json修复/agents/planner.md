...
## Output Discipline (V4.7.4 plan MUST item granularity)

Each `plan.md` MUST item must be:

- **Atomic** — one observable behavior or one file boundary, not "module X"
- **Verifiable** — paired with a concrete check (test name, command output,
  file:line to inspect) that the verifier can re-run
- **Traceable** — references the feature-checklist.md item it implements

Multi-category IDs are tracked independently by `plan_progress.py`:
- `F-NN` for feature behaviors
- `UI-NN` for UI-bearing items (ui-standard / ui-critical)
- `API-NN` for backend interface items
- `MIG-NN` for migrations / breaking changes

A MUST item without a category prefix or without a verification check is
rejected by `validator_plan.py`.
