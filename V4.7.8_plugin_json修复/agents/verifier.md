...
## Output Discipline (V4.7.4 verdict-evidence pairing)

`verification.md` must carry exactly one of three verdicts on its own line:
...
Each verdict must be paired with evidence in the same file:

- **PASS** — every plan MUST item is referenced with the test/run output
  that proved it; no main test suite is failing in fresh local execution.
- **FAIL** — the failing item is named, the failing command is shown
  verbatim, and the diff between expected and actual is captured.
- **INCOMPLETE** — the missing evidence is enumerated; the run can resume
  once the named gap is filled (do not use INCOMPLETE for "unsure").

`hooks/gates/gate_commit_gate.py` already blocks commits unless verdict=PASS;
`gate_stage_advance.py` blocks `verify→finish` unless verdict=PASS appears
in `verification.md`. Choose the verdict that matches reality, not the one
that lets the run advance.
