# SuperTeam V4.7.8 — hotfix · plugin.json 内嵌 ASCII 引号致 manifest 解析失败

**发布日期**: 2026-04-26
**前置版本**: V4.7.7_状态机正确性
**类型**: Hotfix（V4.7.7 发布事故修复）

## 触发事件

V4.7.7 push 后用户跑 `/plugin marketplace update superteam` + `/reload-plugins`：

```
Reloaded: 5 plugins · 2 skills · 8 agents · 0 hooks · 1 plugin MCP server
...
```
Plugin superteam has a corrupt manifest file at
.../plugin.json
JSON parse error: JSON Parse error: Unrecognized token 暂
```

## 根因

V4.7.7 `plugin.json` description 字段内嵌 ASCII 双引号（U+0022）：

```
... 明确 X 暂停 X 与 X 结束 X 两种退出语义 ...    （此处 X 实际为 ASCII 双引号）
```

ASCII 双引号在 JSON string value 内部直接终结字符串，JSON parser 把其后第一个非空白字符（`暂`）当作期待的逗号位置——抛 "Unrecognized token 暂"。

影响面：**整个 superteam plugin 失效**——hook (0/8 注册)、agent (8/25 失活)、skill 全失。V4.7.7 的全部努力（真状态机改造 / pause-resume / AST 纯净测试 / 76 测试全绿）被一行字符抹掉。

## 修复

仅一处改动 — `V4.7.8_plugin_json修复/.claude-plugin/plugin.json`：

| 字段 | V4.7.7 | V4.7.8 |
|---|---|---|
| `version` | 4.7.7 | 4.7.8 |
| `description` | 含 ASCII 双引号 | 改写为不含任何内嵌 ASCII 双引号的纯叙述 |

代码（hooks / mode_state / stop / 测试）与 V4.7.7 **逐字节一致**。

## 防回潮

V4.7.8 push 前已用 `python -c "import json; json.loads(...)"` 验证 `plugin.json` + `marketplace.json` 均解析通过。后续可选：项目根 `.git/hooks/pre-commit` 装 `python -m json.tool` 校验 staged 的 `*.json` 文件。本版本不强加，留给用户按需启用。

## 兼容性

V4.7.7 全部行为与文件保留：

- stop hook 单字段判定 (`project_lifecycle ∈ running/paused/ended`)
- mode_state.py: `pause()` / `resume()` / `project_lifecycle()` / 公开 `write_mode()`
- SessionStart legacy migration
- skills: `pause` / `resume`
- mode_cli.py: `pause` / `resume` 子命令
- AST 纯净性测试 + lifecycle 行为测试
- V4.7.6 三个兼容层补丁（多文件 plan/ + slug 双源 + verify.md 命名）
- V4.7.5 状态机文件 BLOCK 列表

## 测试

V4.7.7 的 76 项测试在 V4.7.8 上同样全绿（代码未改）。

```
python tests/run_tests.py                      # 45/45
python tests/test_or_self_stop_v475.py         # 15/15
python tests/test_v476_compat.py               # 7/7
python tests/test_v477_stop_purity.py          # 2/2
python tests/test_v477_lifecycle.py            # 7/7
```
...
V4.7.6 / V4.7.7 的 marketplace.json description 没有内嵌引号，相安无事；V4.7.7 plugin.json 因为加重语气插了一对 ASCII `"` 就全盘崩溃。**JSON 字符串内部禁止任何未转义 ASCII 双引号是铁律，没有例外**——hotfix 也不能再犯。

---

# SuperTeam V4.7.7 — stop hook 真状态机 · 单字段判定

**发布日期**: 2026-04-26
**前置版本**: V4.7.6_V475回归修复
**类型**: Patch（stop hook 决策概念正名 · 在 V4.7.6 兼容层补丁之上做收口）

## 一句话

V4.7.5 把 stop hook 改成"状态机驱动"是方向对了 — 但执行时把 `current_stage` 和 `is_subagent_running` 也算进了"状态机"，这两个字段描述**瞬时进度**，不是项目生命周期。V4.7.7 还原概念边界：mode.json 加单字段 `project_lifecycle ∈ {running, paused, ended}`，stop hook 只读这一个字段决定阻拦/放行；保留 V4.7.5 的状态机文件 BLOCK 列表和 V4.7.6 的全部兼容层补丁。

## 状态机定义（新）

| 命令 / 事件 | project_lifecycle | stop hook 行为 |
|---|---|---|
| `/superteam:go` | running | BLOCK（OR 不能自停） |
| `/superteam:pause` (新) | paused | ALLOW |
| `/superteam:resume` (新) | running | BLOCK |
| `/superteam:end` | ended | ALLOW |
| phase 7 finish 自动完成 | ended | ALLOW |
| `stop_hook_active=true`（运行时安全阀）| 任意 | ALLOW |

## 修复

### 改动 1 · stop.py 单字段判定

`_or_self_stop_check` 函数**整体替换**（3 行核心逻辑）：
...
def _or_self_stop_check(payload):
    ...
    if mode_state.project_lifecycle() == "running":
        return False, "...self-stop block..."
    return True, ""
```

删除：`_EXEC_CLASS_STAGES` 常量、`from hooks.lib import state` 导入、stage 检查、`is_subagent_running` 检查、stage-specific 文案。

### 改动 2 · mode_state.py 三态字段 + pause/resume

- 新增 `project_lifecycle()` 函数（单字段读取，缺省 "ended" 安全默认）
- 新增 `pause(by="user")` / `resume()` 函数（写 paused/running 并管理 `paused_at`/`paused_by`）
- 新增公开 `write_mode(data)` 原子写入 helper（迁移 / pause / resume 共用）
- `enter()` 加写 `project_lifecycle="running"` + `paused_at=None` + `paused_by=None`
- `end()` 加写 `project_lifecycle="ended"`

保留 `is_or_active` / `is_subagent_running` / `mode_health` 等老函数（其他模块仍引用，仅 stop.py 不再调用）。

### 改动 3 · SessionStart 自动迁移老 mode.json

`hooks/session/session_injection.py` 新增 `_migrate_legacy_lifecycle()` 在 `run()` 开头执行：

| 老 mode.json 状态 | 迁移后 project_lifecycle |
|---|---|
| `ended_at` 已设 | ended |
| `mode == "active"` | running |
| 其他 | ended（安全默认） |

V4.7.6 项目升 V4.7.7 后第一次 SessionStart 自动补字段，无需用户操作。

### 改动 4 · pause/resume 命令

新增：

- `skills/pause/SKILL.md` → `mode_cli.py pause`
- `skills/resume/SKILL.md` → `mode_cli.py resume`
- `commands/cli/mode_cli.py` 加 `cmd_pause` / `cmd_resume` 子命令 + argparse 注册

### 改动 5 · AST 纯净性测试 + 行为测试

- `tests/test_v477_stop_purity.py`（2 case）：AST 扫描 `_or_self_stop_check` 函数体，禁止引用 V4.7.5 时代的瞬时字段（`is_subagent_running` / `current_stage` / `_EXEC_CLASS_STAGES` / `turn_id` 等共 19 个 banned 名）；同时检查 stop.py 模块级别已无 `_EXEC_CLASS_STAGES` 残留。
- `tests/test_v477_lifecycle.py`（7 case）：lifecycle 三态行为 + stop_hook_active 安全阀 + pause CLI + 老 mode.json 自动迁移两条路径。

## 兼容性

- 完全保留 V4.7.6 §3.1/3.2/3.3 三个兼容层补丁（多文件 plan/ + slug 双源 + verify.md 命名）
- 完全保留 V4.7.5 状态机文件 BLOCK 列表（mode.json / active-subagent.json / *-log.jsonl 任何 agent 不得 Edit/Write）
- 完全保留 gate_stage_advance / stop_finish_guard / 其他 V4.7.x 机制

V4.7.5 既有 17 项 stop 测试中：
- A3（stage=clarify allow）+ A7（subagent-running allow）**删除**：V4.7.7 不再读这两字段
- A4/A5/A6（stage=execute/review/verify BLOCK）**保留**：lifecycle=running 同样 BLOCK，仅断言文案改为 V4.7.7 lifecycle 信息
- A9（finish stage）**重写**：用 lifecycle=ended 路径替代旧的 "finish 不在 EXEC_CLASS" 路径

## 总测试

```
python tests/run_tests.py                      # 45/45 V4.6 e2e
python tests/test_or_self_stop_v475.py         # 15/15 V4.7.5 baseline (V4.7.7 适配)
python tests/test_v476_compat.py               # 7/7 V4.7.6 兼容性
python tests/test_v477_stop_purity.py          # 2/2 V4.7.7 AST 纯净性
python tests/test_v477_lifecycle.py            # 7/7 V4.7.7 lifecycle 行为
```
...
> "状态机的字段必须是项目级别的稳定状态。`current_stage` 是七阶段进度，
> `is_subagent_running` 是 specialist 持有 flag — 它们随每次 spawn 翻转，
> 描述的是'正在做什么'而非'项目处于什么生命周期'。把它们塞进 stop 判定
> 等于让瞬时事件控制项目级开关，结构上是错位。"

V4.7.7 = V4.7.5/V4.7.6 严约束 + 概念边界正名。stop hook 现在只读一个字段，
回潮风险由 AST 测试 + pre-commit hook（用户可选装）防住。

## 详情

设计文档见 `D:\claude code\superteam\PLAN-V4.7.7-state-machine-correctness.md`。
