...
def project_lifecycle() -> str:
    """V4.7.7 — single-field state machine for stop-hook decision.

    Returns one of: 'running', 'paused', 'ended'. Defaults to 'ended' for
    legacy mode.json that predates the field — safe-by-default (stop allowed).
    SessionStart hook upgrades legacy mode.json on first session start.
    """
    return read_mode().get("project_lifecycle", "ended")
