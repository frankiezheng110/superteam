...
Core roles: `orchestrator`, `planner`, `architect`, `executor`, `inspector`, `verifier`
Mandatory support role: `reviewer` (continuity checkpoints in the first three stages + post-run audit)
...
Initialize Reviewer trace at `.superteam/reviewer/traces/<task-slug>.jsonl` when `clarify` starts. Emit trace events per `framework/reviewer.md` throughout the run.
...
- `review`: create `review.md`, consume `polish.md`, use `inspector` with specialist profiles as needed, enforce the TDD gate for code-changing work, apply UI quality gate when required, write `handoffs/05-review-to-verify.md`
...
- `finish`: only after `PASS`, trigger Reviewer analysis to generate the team-duty report, create `finish.md` with trace summary, Reviewer summary, and aesthetic quality assessment when relevant, create `retrospective.md`, write `handoffs/06-verify-to-finish.md`
...
1. A trace file at `.superteam/reviewer/traces/<task-slug>.jsonl` — started at `clarify`, appended throughout
2. Reviewer reports at `.superteam/reviewer/reports/<task-slug>-report.html` and `.superteam/reviewer/reports/<task-slug>-report.md` — generated before `finish.md`
3. All Reviewer problem records must be acknowledged in `finish.md` (`acknowledged` / `addressed` / `disputed`)

This is non-negotiable. No run completes without a Reviewer report.

## Completion Standard

All seven stages must be represented in run artifacts, `polish.md` must exist before `review`, verification must have a real `PASS`, meaningful runs must include `retrospective.md`, and a Reviewer report must exist. For UI work, `ui-intent.md` with complete aesthetic contracts and anti-pattern gate clearance are also required.
