...
- run a general `inspector` pass by default
- activate the inspector's specialist profiles as needed: critic (high-risk reasoning), security (auth/secrets/permissions), acceptance (user-facing criteria), tdd (test discipline), socratic (hidden assumptions)
...
- check whether reviewer continuity and trace expectations were actually maintained across the first three stages
