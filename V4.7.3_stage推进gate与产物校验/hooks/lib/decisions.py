...
def emit_user_prompt_context(context: str) -> None:
    """V4.7.0 — UserPromptSubmit additionalContext injection.

    Each user message is the safety-net reinjection point that survives
    auto-compact, usage-limit resume, and unexpected session continuations
    where SessionStart did not fire.
    """
    context = _truncate(context, MAX_SESSION_CONTEXT)
    payload = {
        "hookSpecificOutput": {
