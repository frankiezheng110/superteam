...
`activity-trace.md` is the human-readable continuity trail for the first three stages. It must agree with the Inspector trace on stage order and key decisions.
...
```json
{
  ...
  "active_specialists": ["inspector", "designer"],
  "specialist_reason": "review stage requires UI quality gate plus general quality gate",
  "inspector_trace_path": ".superteam/inspector/traces/example-task.jsonl",
  "inspector_trace_events": 42,
  "inspector_report_status": "pending",
  "inspector_open_problems": 0,
...
Inspector report status should be one of:
...
- the existing Inspector trace file for the run, to restore event history context
- any open Inspector improvement records from the improvement backlog
...
- Inspector trace event count, report status, and open problem count when relevant
