...
A. stop.py::_or_self_stop_check — V4.7.7 single-field read
   - mode missing / mode=ended                       → allow
   - project_lifecycle=running                       → BLOCK
   - stop_hook_active=true                           → allow (runtime safety valve)
   - finish completed → lifecycle=ended              → allow
...
def case_a4_execute_lifecycle_running_blocks() -> None:
    ...
    try:
        ...
        expect(
            "A4 reason mentions V4.7.7 lifecycle",
            "project_lifecycle" in res.reason or "V4.7.7" in res.reason,
            res.reason,
        )
    finally:
        destroy_workspace(ws)
...
def case_a8_stop_hook_active_allows() -> None:
    """stop_hook_active=true → allow (runtime safety valve, preserved across versions)."""
    ws = make_workspace()
    try:
        ws.init(slug="t1", stage="execute")
