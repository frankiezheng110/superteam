...
def current_slug() -> str:
    """V4.7.6 优先读 mode.json.active_task_slug (V4.7 phase 粒度),
    fallback 到 current-run.json.task_slug (V4.6 项目粒度)。

    历史: V4.7.0 引入 mode.json.active_task_slug 时未自动同步
    current-run.json.task_slug, 导致 V4.6→V4.7 升级项目两值漂移。
    本次改让 current_slug() 始终返回当前 phase 的真实 slug。

    函数内 import 避免循环依赖 (mode_state import state)。
    直接读 mode.json 不走 mode_state.active_task_slug() 的
    is_or_active() 早返回 — slug 字段在 ended/corrupt 状态下也可能可读,
    保持 hook 在边缘状态下的容错能力。
    """
    from . import mode_state
    md = mode_state.read_mode()
    slug = md.get("active_task_slug")
    if slug:
        return slug
