...
from ..lib import mode_state, state
...
def check(tool_input: dict[str, Any]) -> tuple[bool, str]:
    ...
    #
    # V4.7.3 — exempt current-run.json under OR mode: the main session must
    # be able to advance current_stage / record next_action / mark blockers,
    # and gate_stage_advance.check() now polices the dangerous part of that
    # write (illegitimate current_stage transitions). Other state/* files
    # remain hook-only.
    if STATE_DIR_RE.search(fp):
        norm = fp.replace("\\", "/")
        if mode_state.is_or_active() and norm.endswith(".superteam/state/current-run.json"):
            return True, ""
