...
def main() -> None:
    ...
    if tool == "Agent":
        ...
        # V4.7.1 — only SuperTeam specialists open the subagent-running
        # window. Spawning a non-SuperTeam subagent (general-purpose,
        # Explore, etc.) must not give the main session a bypass on
        # gate_main_session_scope, so we don't mark started for them.
        if mode_state.is_or_active() and agent.startswith("superteam:"):
        ...
        if not ok:
            decisions.emit_block(reason)
        # V4.7.3 stage-advance gate — fires only when current-run.json is the
        # write target and the proposed current_stage differs from disk.
        ok, reason = gate_stage_advance.check(tool_input)
        if not ok:
            decisions.emit_block(reason)
