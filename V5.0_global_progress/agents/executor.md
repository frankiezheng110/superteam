...
## Output Discipline (V4.7.4 execution.md content rules)

`execution.md` is the canonical record of what you did. For each MUST item:

- Open a `## Feature <id>` section verbatim matching the plan.md MUST id.
- Under it, log the red→green→refactor sequence:
  - **RED** — failing test command + the failure output
  - **GREEN** — the same test passing + relevant code locations changed
    (use `path/file.ext:LINE` form so reviewer can navigate)
  - **REFACTOR** — optional cleanup, with what was simplified and why
- If the feature ends BLOCKED, write `Status: BLOCKED` plus the reason
  and what input is needed to unblock; do NOT silently skip.
- If you used a TDD waiver (orchestrator-issued), reference the
  `tdd_exception` field in `current-run.json` and quote the waiver reason.

Three consecutive BLOCKED features halt the execute stage
(`validator_execution.py::check_consecutive_blocked`) — escalate sooner.
