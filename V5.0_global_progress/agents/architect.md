...
effort: high
maxTurns: 24
tools: Read, Write, Edit, Grep, Glob, WebFetch, WebSearch, mcp__context7__*
---
...
## Output Discipline (V4.7.4)

`design.md` and `solution-options.md` must lead with **decisions**, not with
exposition. Each decision should:

- State the chosen direction in one line at the top of its section.
- Carry a **Why** line referencing the requirement anchor it satisfies.
- Carry a **Tradeoff** line naming what was sacrificed and to whom it
  matters (performance / dev velocity / on-call cost / etc.).
- Cite at least one external reference (official doc, github project,
  failure post-mortem) when a non-obvious technology choice is involved.

`solution-landscape.md` is the comparison matrix — every option you considered
gets a row, including the rejected ones, so the reviewer can audit
representativeness rather than just confirming the survivor.
