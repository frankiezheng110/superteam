...
from ..lib import compat, decisions, global_progress, mode_state, plan_progress, project_state, state
...
def _migrate_legacy_lifecycle() -> None:
    """V4.7.7 — backfill project_lifecycle on legacy mode.json.

    Pre-V4.7.7 mode.json has only mode/ended_at fields. V4.7.7's stop
    hook reads project_lifecycle; absent the field it defaults to 'ended'
    (stop allowed), which would silently disable the OR self-stop guard
    on projects entered before the upgrade. This SessionStart pass
    promotes legacy records once:
        ended_at set        → ended
        mode == 'active'    → running
        otherwise           → ended (safe default)
    """
    md = mode_state.read_mode()
    if not md or "project_lifecycle" in md:
        return
    if md.get("ended_at"):
        md["project_lifecycle"] = "ended"
    elif md.get("mode") == "active":
        md["project_lifecycle"] = "running"
    else:
        md["project_lifecycle"] = "ended"
    mode_state.write_mode(md)


def _project_banner() -> str | None:
    """V4.7.10 — at SessionStart, surface the project layer so the OR sees
    the milestone roadmap before deciding next moves.

    No-ops when project.md is absent (legacy projects keep the V4.7.9
    behavior unchanged). Output stays terse on purpose: a one-line header
    plus the active and next milestones, plus a pointer to the file. The
    full table lives on disk; the OR can read it on demand.
    """
    ...
    cur = fm.get("current_milestone_slug") or "(unset)"
    nxt = project_state.next_pending_milestone()
    nxt_str = (
        f"{nxt.get('phase_slug','?')} ({nxt.get('version','?')})"
        if nxt and nxt.get("phase_slug") and nxt.get("phase_slug") != cur
        else "(none — all milestones DONE or current is last)"
    )
    return (
        "【SuperTeam V4.7.10 project】\n"
        f"project: {name} · target: {target} · status: {status}\n"
        f"current_milestone: {cur} · next_pending: {nxt_str}\n"
        "完整 milestone 表见 .superteam/project.md。"
        "phase finish 自动标该 milestone DONE 但不结束项目;"
        "全部 DONE 后用户运行 /superteam:project-complete 才结束。"
    )
...
def run() -> str:
    ...
    # V5.0 — health alarm has priority over status banner.
    health = mode_state.mode_health()
    if health in (mode_state.MODE_HEALTH_CORRUPT, mode_state.MODE_HEALTH_UNKNOWN_SCHEMA):
        bits = [
        ...
        if health == mode_state.MODE_HEALTH_ACTIVE:
            mode_state.bump_last_verified()
        # V5.0 unified global progress banner — replaces V4.7.10 _project_banner
        # + V4.7.1 _or_mode_banner. Aggregates project / mode / current-run /
        # plan-progress / feature-tdd-state into one render so the OR sees
        # the full hierarchy every turn (套娃 防御 by visibility, not by
        # adding new gates).
        banner = global_progress.render_progress_banner()
        if banner:
            bits.append(banner)
